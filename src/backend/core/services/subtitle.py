...
class SubtitleService:
    ...
    async def stop_subtitle(self, room) -> None:
        ...
        Native captions are ROOM-WIDE (one agent per room), so this stops the
        pipeline for ALL participants — the LinTO takeover semantic. Two levers,
        both room-scoped to ``str(room.id)`` and safe to call when the agent is
        already gone:
