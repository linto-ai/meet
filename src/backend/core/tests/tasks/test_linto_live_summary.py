...
class TestTwakeTextOnly:
    ...
    def test_twake_upload_is_text_only(self, no_twake, fast_resolve):
        ...
        assert any(f.startswith("Transcription_") for f in filenames)
        assert "Plus_de_detail_dans_linto.url" in filenames
        assert any(f.startswith("transcription-") for f in filenames)
        assert not any(f.startswith("Enregistrement_") for f in filenames)
