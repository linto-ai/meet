...
its path, so a user renaming or moving `Meetings` must not make the service
...
def test_pick_live_directory_ignores_trashed_and_files():
    ...
    included = [
        _directory("old", "/Meetings", "2024-01-01T00:00:00Z"),
        _directory("trashed", "/.cozy_trash/Meetings", "2025-01-01T00:00:00Z"),
        {
            "type": "io.cozy.files",
            "id": "file",
...
def test_pick_live_directory_returns_none_when_nothing_matches():
    ...
    trashed = [_directory("t", "/.cozy_trash/Meetings", "2025-01-01T00:00:00Z")]
...
def test_names_follow_the_language_with_english_fallback(recording, language, expected):
    """Supported languages get their labels, anything else falls back to English."""
