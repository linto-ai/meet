...
def _publication_meta(pub_format, room_name, date_str):
    """Return the ``(filename, mime_type)`` of the exported publication file."""
    pub_mime = _PUBLICATION_MIME_TYPES.get(pub_format, "application/octet-stream")
    pub_filename = f"transcription-{room_name}-{date_str}.{pub_format}"
    return pub_filename, pub_mime
...
async def _deliver_to_twake(  # noqa: PLR0913 - delivery needs the full context
    *,
    ...
    """Upload transcript/summary/media/shortcut/document to Twake Drive (soft-fail).

    ``extra_files_provider`` is an optional async callable returning a list of
    ``(filename, content, content_type)`` for the original-media bytes (the
    recording flow uploads the mp4/ogg; the live flow passes ``None`` — text
    only). Returns the Twake Drive folder link, or ``None`` when Twake is not
    configured, the step is already checkpointed, or the upload failed.
    """
...
async def _process_linto_transcription_sync(recording_id):  # noqa: PLR0915
    ...
    pub_filename, pub_mime = _publication_meta(pub_format, room_name, date_str)
    ...
    async def _recording_media_files():
        ...
        if recording.extension == FileExtension.MP4.value:
            files.append(
                (f"Enregistrement_{meeting_time}.mp4", file_content, "video/mp4")
            )
            ...
            files.append((f"Enregistrement_{meeting_time}.ogg", ogg_audio, "audio/ogg"))
        else:
            files.append(
                (f"Enregistrement_{meeting_time}.ogg", file_content, "audio/ogg")
            )
        return files
...
async def _process_bot_live_summary_sync(room_id):
    ...
    meeting_time = meeting_dt.strftime("%d-%m-%Y_%H-%M")
    pub_filename, pub_mime = _publication_meta(pub_format, room_name, date_str)
    summary_preview = _extract_summary_preview(summary_result)
